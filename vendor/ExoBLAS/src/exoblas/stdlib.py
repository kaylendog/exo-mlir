...
from exoblas.exceptions import *
from exoblas.higher_order import *
from exoblas.rc_wrappers import *
...
def unroll_and_jam(proc, loop, factor, unroll=(True, True, True)):
    ...
    return interleave_outer_loop_with_inner_loop(
        proc, loop, inner_loop, factor, unroll=unroll
    )
...
def vectorize(
    ...
    if tail in {"predicate", "cut_and_predicate"}:
        return vectorize_predicate_tail(
            proc, loop, vec_width, precision, mem_type, instructions, patterns, tail, rc
        )
    ...
    for depth, (loop, tile) in enumerate(loops[::-1]):
        if tail == "cut_and_guard":
            if tile is not None:
                ...
                proc = push_scope_in(
                    proc, tail_l.body()[0], depth + min_depth + 1, tile
                )
            else:
                proc = push_scope_in(proc, loop, depth + min_depth + 1)
        elif tail == "guard1":
            if tile is not None:
                proc, (_, inner, _) = divide_loop_(
                    proc, loop, tile, tail="guard", rc=True
                )
                proc = simplify(proc)
                proc = push_scope_in(
                    proc, inner.body()[0], depth + guards + min_depth + 1
                )
                guards += 1
...
def bound_loop_by_if(proc, loop):
    ...
    if is_read(proc, cond_lhs, loop.name()) and cond.op() == "<":
        ...
        is_add(proc, cond_lhs)
        and is_read(proc, cond_lhs.lhs(), loop.name())
        and cond.op() == "<"
    ):
        cut_point = FormattedExprStr(
            "_ + (_ - _)", loop.lo(), cond.rhs(), cond_lhs.rhs()
        )
    else:
        raise BLAS_SchedulingError(err)
            ...
            for lhs, rhs in operands, operands[::-1]:
                if is_literal(proc, lhs) and lhs.value() % e.rhs().value():
                    return rewrite_expr(
                        proc,
                        e,
                        FormattedExprStr("_ * _", lhs.value() / e.rhs().value(), rhs),
                    )
        return proc
    ...
    for src_dim, _ in reversed(list(enumerate(alloc.shape()))):
        for dst_dim, size in reversed(divisions[src_dim][1:]):
            ...
            proc = apply(divide_loop_)(
                proc, loop_nest[src_dim], size, tail="cut_and_guard"
            )
            perm.append(dst_dim)
