...
from exoblas.machines.abstract_vector import *
from exoblas.machines.machine_params import MachineParameters
...
class VEC_AVX512(VEC):
    ...
    def alloc(cls, new_name, prim_type, shape, srcinfo):
        ...
        if not prim_type in vec_types.keys():
            raise MemGenError(
                f"{srcinfo}: AVX512 vectors must be f32/f64/ui16 (for now), got {prim_type}"
            )

        reg_width, C_reg_type_name = vec_types[prim_type]
        if not (shape[-1].isdecimal() and int(shape[-1]) == reg_width):
            raise MemGenError(
                f"{srcinfo}: AVX512 vectors of type {prim_type} must be {reg_width}-wide, got {shape}"
            )
        shape = shape[:-1]
        if shape:
            result = f"{C_reg_type_name} {new_name}[{']['.join(map(str, shape))}];"
...
def get_avx_instrs(VEC_MEM):
    ...
    for precision_l in 32, 64:
        ...
        itype_sfx = (
            "epi32" if precision == "f32" else ("epi64x" if bit_l == 256 else "epi64")
        )
        mm_pfx = f"mm{bit_l}"
        ...
        def make_stmt(instr, pfx=False):
            ...
            if VEC_MEM is VEC_AVX2:
                ...
                if (
                    pfx
                    and "set1" not in instr
                    and "setzero" not in instr
                    and not DONT_PREDICATE
                ):
                    pat = "mm512"
                    index = instr.find(pat)
                    instr = (
                        instr[: index + len(pat)] + "_mask" + instr[index + len(pat) :]
                    )
                    if "fmadd" not in instr:
                        ...
                        instr = (
                            instr[: index + len(pat)]
                            + f"{mm_pfx}_prefix_mask_{itype_sfx}({{m}}), "
                            + instr[index + len(pat) :]
                        )
                return f"{{dst_data}} = {instr};"

        # Load Family
        avx2_vec_op, avx2_vec_op_pfx = specialize_vec_op(
            vec_load, vw, precision, VEC_MEM
        )
        yield make_instr(
            avx2_vec_op, f"{{dst_data}} = _{mm_pfx}_loadu_{type_sfx}(&{{src_data}});"
        )
        mload_op = "maskload" if VEC_MEM is VEC_AVX2 else "maskz_loadu"
        mask = (
            f"{mm_pfx}_prefix_mask_{itype_sfx}({{m}})"
            if VEC_MEM is VEC_AVX2
            else f"{mm_pfx}_prefix_mask_{itype_sfx}({{m}})"
        )
        args = (
            f"&{{src_data}}, {mask}"
            if VEC_MEM is VEC_AVX2
            else f"{mask}, &{{src_data}}"
        )
        yield make_instr(
            avx2_vec_op_pfx, f"{{dst_data}} = _{mm_pfx}_{mload_op}_{type_sfx}({args});"
        )

        avx2_vec_op, avx2_vec_op_pfx = specialize_vec_op(
            vec_load_bck, vw, precision, VEC_MEM
        )
        yield make_instr(
            avx2_vec_op, f"{{dst_data}} = _{mm_pfx}_loadu_{type_sfx}(&{{src_data}});"
        )
        mload_op = "maskload" if VEC_MEM is VEC_AVX2 else "maskz_loadu"
        mask = (
            f"{mm_pfx}_suffix_mask_{itype_sfx}({{m}})"
            if VEC_MEM is VEC_AVX2
            else f"{mm_pfx}_suffix_mask_{itype_sfx}({{m}})"
        )
        args = (
            f"&{{src_data}}, {mask}"
            if VEC_MEM is VEC_AVX2
        ...
        # Store Family
        avx2_vec_op, avx2_vec_op_pfx = specialize_vec_op(
            vec_store, vw, precision, VEC_MEM
        )
        yield make_instr(
            avx2_vec_op, f"_{mm_pfx}_storeu_{type_sfx}(&{{dst_data}}, {{src_data}});"
        )
        ...
        avx2_vec_op, avx2_vec_op_pfx = specialize_vec_op(
            vec_store_bck, vw, precision, VEC_MEM
        )
        yield make_instr(
            avx2_vec_op, f"_{mm_pfx}_storeu_{type_sfx}(&{{dst_data}}, {{src_data}});"
        )
        ...
        # Add_red Family
        avx2_vec_op, avx2_vec_op_pfx = specialize_vec_op(
            vec_add_red, vw, precision, VEC_MEM
        )
        ...
        for instr, name in binops:
            avx2_vec_binop, avx2_vec_binop_pfx = specialize_vec_op(
                instr, vw, precision, VEC_MEM
            )
        ...
        # Fmadd Family
        avx2_vec_op, avx2_vec_op_pfx = specialize_vec_op(
            vec_fmadd_red, vw, precision, VEC_MEM
        )
        fmadd_red_instr = (
            f"_{mm_pfx}_fmadd_{type_sfx}({{src1_data}}, {{src2_data}}, {{dst_data}})"
        )
        ...
        for vec_fmadd in vec_fmadd1, vec_fmadd2:
            avx2_vec_op, avx2_vec_op_pfx = specialize_vec_op(
                vec_fmadd, vw, precision, VEC_MEM
            )
        ...
        # Abs Family
        avx2_vec_op, avx2_vec_op_pfx = specialize_vec_op(
            vec_abs, vw, precision, VEC_MEM
        )
        if VEC_MEM is VEC_AVX2:
            abs_mask = (
                "0x7" + "F" * 7 if precision == "f32" else "0x7" + "F" * 15 + "LL"
            )
        ...
        if precision == "f64":
            avx2_vec_op, avx2_vec_op_pfx = specialize_vec_op(
                load_f32_cvt, vw, precision, VEC_MEM
            )
            if VEC_MEM is VEC_AVX2:
                yield make_instr(
                    avx2_vec_op,
                    f"{{dst_data}} = _{mm_pfx}_cvtps_pd(_mm_loadu_ps(&{{src_data}}));",
                )
                yield make_instr(
                    avx2_vec_op_pfx,
                ...
                yield make_instr(
                    avx2_vec_op,
                    f"{{dst_data}} = _{mm_pfx}_cvtps_pd(_mm256_loadu_ps(&{{src_data}}));",
                )
                yield make_instr(
                    avx2_vec_op_pfx,
