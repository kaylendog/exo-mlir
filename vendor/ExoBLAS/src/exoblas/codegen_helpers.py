...
def simplify_triang_loop(proc, block, mapping):
    ...
    def extract_cut_point(proc, loop):
        ...
        cut_point = (
            cond.lhs().rhs().rhs()
            if mapping["Uplo"] == CblasUpperValue
            else cond.rhs().rhs().rhs()
        )
...
def variants_generator(
    blas_op, opt_precisions=("f32", "f64"), targets=("avx2", "avx512", "neon")
):
        ...
        for precision in ("f32", "f64"):
            ...
            if precision in opt_precisions and Machine.name in targets:
                if not calls:
                    ...
                    stride_1 = blas_op(
                        stride_1, loop, precision, Machine, *args, **kwargs
                    )
                else:
                    for call, mapping in calls:
                        ...
                        filtered_mapping = dict(
                            filter(lambda p: check(p[0]), mapping.items())
                        )
                        subproc = blas_op(
                            subproc,
                            loop,
