...
# CHECK-NEXT: func.func @fixed_matmul(%0 : memref<16x16xf32>, %1 : memref<16x16xf32>, %2 : memref<16x16xf32>) {
@proc
def fixed_matmul(C: f32[16, 16] @ DRAM, A: f32[16, 16] @ DRAM, B: f32[16, 16] @ DRAM):
    ...
    for i in seq(0, 16):
        ...
        for j in seq(0, 16):
            # CHECK-NEXT:memref.store %8, %0[%9, %10] : memref<16x16xf32>
            C[i, j] = 0.0
            # CHECK-NEXT: scf.for %11 = %4 to %6 step %7 {
            for k in seq(0, 16):
                # CHECK-NEXT: %12 = memref.load %1[%9, %11] : memref<16x16xf32>
                # CHECK-NEXT: %13 = memref.load %2[%11, %10] : memref<16x16xf32>
                # CHECK-NEXT: %14 = arith.mulf %12, %13 : f32
                # CHECK-NEXT: %15 = memref.load %0[%9, %10] : memref<16x16xf32>
                # CHECK-NEXT: %16 = arith.addf %15, %14 : f32
                # CHECK-NEXT: memref.store %16, %0[%9, %10] : memref<16x16xf32>
                C[i, j] += A[i, k] * B[k, j]
            # CHECK-NEXT: }
        # CHECK-NEXT: }
    # CHECK-NEXT: }
