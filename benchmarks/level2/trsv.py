...
def schedule_t(proc, i_loop, precision, machine, rows_factor, cols_factor):
    ...
    proc, (_, oi_loop, t_loop) = divide_loop_(
        proc, i_loop, rows_factor, tail="cut", rc=True
    )
    ...
    proc = rewrite_expr(
        proc,
        j_loop.hi().lhs(),
        f"(n - ({rows_factor} * io + n % {rows_factor}) - 1) / {vw}",
    )
    j_loop_tail = j_loop.next()
    proc = reorder_stmt_forward(proc, j_loop)
    proc = optimize_level_1(
        proc, j_loop_tail, precision, machine, 1, vec_tail="predicate"
    )
    ...
    proc = optimize_level_1(
        proc, tail_loop, precision, machine, rows_factor * cols_factor
    )
    return proc
