...
def schedule_macro(
    gemm_mk, precision, machine, max_M, max_N, max_K, m_r, n_r_fac, TransA, TransB
):
    ...
    gemm_mk, cursors = pack_mem(
        gemm_mk, i_loop, "AT" if isTransA else "A", packed_A_shape, "packed_A", rc=1
    )
    ...
    gemm_mk, cursors = pack_mem(
        gemm_mk, i_loop, "BT" if isTransB else "B", packed_B_shape, "packed_B", rc=1
    )
    ...
    if not isTransB:
        ...
        gemm_mk = optimize_level_1(
            gemm_mk, gemm_mk.find_loop("i1i"), precision, machine, 1
        )
...
def schedule_tiled(
    ...
    gemm_macro = schedule_macro(
        gemm, precision, machine, M_tile, N_tile, K_tile, m_r, n_r_fac, TransA, TransB
    )
    ...
    gemm_tiled = apply(repeate_n(reorder_loops))(
        gemm_tiled, gemm_tiled.find_loop("ki", many=True), n=1
    )
...
variants_generator(schedule_gemm, targets=("avx2", "avx512"))(
    gemm, "i", globals=globals()
)
