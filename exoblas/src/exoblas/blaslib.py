...
def _optimize_level_2_skinny(
    proc, outer_loop, precision, machine, skinny_factor, cols_factor
):
    inner_loop = get_inner_loop(proc, outer_loop)
    vw = machine.vec_width(precision)

    reused_vector = list(get_reused_vectors(proc, inner_loop))[
        0
    ]  # We will only support one vector for now (level 2)
    proc = simplify(round_loop(proc, inner_loop, vw, up=machine.supports_predication))
    proc, (alloc, load, _, store) = auto_stage_mem(
        proc, outer_loop, reused_vector.name(), rc=True
    )
    proc = simplify(proc)
    ...
    proc = specialize(
        proc,
        proc.body(),
        [f"{cond_lhs} == {vw * i}" for i in range(1, skinny_factor + 1)],
    )
    proc = unroll_loops(simplify(proc))
    return cleanup(proc)


def adjust_level_2_triangular(
    proc, outer_loop, precision, machine, rows_factor, round_up=None
):
...
def optimize_level_2_general(
    ...
    **kwargs,
    ...
    proc, (outer_loop,) = adjust_level_2_triangular(
        proc, outer_loop, precision, machine, rows_factor, round_up
    )
    ...
    **kwargs,
    ...
    if skinny_factor and machine.supports_predication:
        ...
        proc = specialize(
            proc,
            outer_loop,
            f"{expr_to_string(reused_vector.shape()[0])} <= {skinny_factor[0] * vec_width}",
        )
    ...
    def wrapper(proc, *args, **kwargs):
        ...
        for p, sched_p in gcache:
            ...
            if str(proc_renamed) == str(p_renamed):
                proc, success = attempt(replace)(
                    proc, proc.body(), p, quiet=True, rs=True
                )
                if not success:
                    continue
...
def schedule_compute(proc, i_loop, precision, machine, m_r, n_r_fac):
    ...
    proc = apply(attempt(lift_scope))(
        proc, filter_cursors(is_if)(proc, nlr_stmts(proc))
    )
